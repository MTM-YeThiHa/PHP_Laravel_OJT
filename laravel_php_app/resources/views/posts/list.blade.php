						...
						<div class="row d-flex justify-content-end">
							<div class="col-auto d-flex">
								...
								<form method="post" action="{{ route('postSearch') }}" class="d-flex">
									@csrf
									<div class="mx-1">
										<input class="search-input mb-2 form-control" name="search" type="text" id="search-keyword-sm" value="{{request ('search')}}" placeholder="Search..." />
									</div>
									<div class="mx-1">
								...
								@else
								<button class="btn btn-success header-btn" disabled>{{ __('Download') }}</button>
								@endif
								@else
							...
							<form class="col-md-4 d-flex justify-content-start" style="align-items: baseline;" method="GET" action="{{ url()->current() }}">
								<div class="pagination d-flex justify-content-center align-items-center">
									...
									<select class="form-select" id="perPage" name="perPage" onchange="this.form.submit()" style="width: 70px;">
										...
										<option value="12" {{ $postList->perPage() == 12 ? 'selected' : '' }}>12</option>
										<option value="15" {{ $postList->perPage() == 15 ? 'selected' : '' }}>15</option>
										<option value="20" {{ $postList->perPage() == 20 ? 'selected' : '' }}>20</option>
										<option value="25" {{ $postList->perPage() == 25 ? 'selected' : '' }}>25</option>
									</select>
								</div>
							</form>
